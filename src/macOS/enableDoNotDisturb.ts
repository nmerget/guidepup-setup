...
import { handleWarning } from "../logging";
...
const isDndOptional = process.argv.includes("--dnd-optional");
...
export async function enableDoNotDisturb() {
  ...
  try {
    ...
    if (isDndOptional) {
      handleWarning(ERR_MACOS_FAILED_TO_ENABLE_DO_NOT_DISTURB, e.message);
    } else {
      throw new Error(
